...
| 2, "both … -worktrees … exist under …" | Both `skimmer-worktrees` and `manta-worktrees` exist under `--org-dir` — whether because the destination parent was created independently, or because a migrated host's legacy worktree parent was never merged in | Resolve by hand — decide which worktree parent is authoritative, merge or remove the other — then re-run |
