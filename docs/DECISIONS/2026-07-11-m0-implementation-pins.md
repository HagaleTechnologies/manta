...
(19 items — 1-15 were made at plan-writing time; 16-19 were added during
execution as real bugs were found, root-caused, and fixed).
...
instead). The Kaiser prototype designer (`skimmer-dsp::proto`) is new code —
coppa has no FIR designer to reuse.

### Prototype filter tap snapshot (Task 7)

`crates/skimmer-dsp/src/proto.rs`'s `reference_taps_pinned` test pins
