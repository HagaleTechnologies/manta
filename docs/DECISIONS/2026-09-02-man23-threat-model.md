...
| MAN-61 | `ConnectionLimiter` doesn't actually prevent connection-count exhaustion — a quiet-forever client (which the JSON protocol explicitly permits by design) can hold a permit indefinitely, and 512 such clients deny every legitimate connection | P2 — corrects an over-broad "Already covered" disposition from review round 1; a real, currently-open availability gap on manta's two public-facing surfaces |
...
- No application-code changes were made from this ticket directly — every
  ...
  MAN-58, MAN-59, MAN-60, MAN-61) per the ticket's own invariant against
  folding new findings into this ticket's scope.
