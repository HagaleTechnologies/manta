...
A structured, STRIDE-organized adversarial pass over the surfaces named in
MAN-23's own scope (below) — **not** every network-facing surface manta
exposes; KiwiSDR and the other pre-existing input drivers are explicitly
excluded (review round 4 correction: an earlier draft's opening line still
claimed the broader "every surface" scope after the Scope section below it
had already been narrowed in round 1 — same finding, remediation left this
sentence unfixed). This pass complements MAN-22's baseline input-hygiene
fixes with the deeper "is this the right design" pass MAN-22's own ticket
explicitly deferred to this one. Every finding below is given
...
|---|---|---|---|
| 1 | Tampering / DoS | A single malformed UDP packet (correct length, bad USB sync bytes) propagated a fatal `Err` all the way out of `manta-engine::listen`'s read loop via unmatched `?` — one crafted packet killed the whole process. **Also covers a related, initially-separate finding (review round 4): a sustained flood of short/wrong-length datagrams hit `Ok(_) => continue` with no bound at all** — `consecutive_timeouts` only advances on `WouldBlock`/`TimedOut`, never on a merely-short datagram, so `pump_one_packet` could loop indefinitely holding `Inner`'s shared mutex (stalling every DDC on the device) without ever timing out | **Fixed for the acute case; a bounded residual is accepted risk (review round 5 correction)** — PR #75 has since merged, confirmed via `git log` on this branch after syncing with `main`. Both the bad-sync-bytes case and the short/wrong-length-datagram flood case are bounded by the same fix: `MAX_CONSECUTIVE_MALFORMED` counts *both* discard paths and, after 10,000 consecutive malformed packets, `bail!`s a clear `Err` rather than looping/holding the mutex forever. **What this does NOT do**: that `Err` still propagates through `IqSource::read` → `manta_engine::listen` → the CLI's `listen_result?`, terminating the whole process — a sustained-enough flood (10,000+ consecutive malformed packets, not one) still causes a process-level DoS, just one requiring vastly more sustained attacker effort than the pre-fix single-packet case. This is **the same accepted design pattern already established for `MAX_CONSECUTIVE_TIMEOUTS`** (a device producing no valid data at all, whether via silence or via noise, is architecturally treated as fatal after a bound — matching `KiwiIqSource`'s own convention) — not a new gap introduced by this fix, and not something this PR should change unilaterally. Worth explicit reconsideration once MAN-13 (multi-source) exists: today one source terminating is equivalent to the whole daemon stopping (only one source runs at a time — `manta-cli/src/main.rs` hardcodes `ddc_count: 1`), but once multiple sources can run concurrently, one attacked/misbehaving source taking down every other source's daemon becomes a materially worse availability story than it is today — covered by this doc's own "re-run this pass once MAN-13 lands" caveat, not a new ticket. Regression-tested against a live loopback socket for both the bad-sync-bytes and short/wrong-length-datagram cases, both verified to fail against the pre-fix code |
...
| 5 | Tampering / DoS | Structurally invalid (not just oversized) WebSocket frames | **Already covered** — tungstenite's server-side parser rejects these; MAN-22 adds the first test that actually sends non-tungstenite-constructible garbage bytes post-handshake to prove it, rather than only exercising well-formed-but-hostile messages built through tungstenite's own client API. **Scope note (review round 6): this covers post-upgrade frames only** — the WS *handshake itself* is a distinct code path with no size bound, see finding 5a below |
| 5a | DoS | The 16 KiB `max_message_size`/`max_frame_size` config (finding 5) governs post-upgrade WS frames only — the HTTP upgrade handshake itself, which `tokio_tungstenite::accept_async_with_config` parses to complete the handshake, has no exposed size bound in that same config. `HANDSHAKE_TIMEOUT` (10s) bounds *time*, not *data volume* — a client streaming an unterminated, oversized upgrade request for up to 10s could force tungstenite to buffer significant memory per connection, multiplied by up to 512 concurrently-admitted connections. Found in review round 6 | **Filed — MAN-63** |
...
| 11a | DoS | `ConnectionLimiter`'s 64-connection cap and 30s header deadline bound *simultaneous slow* metrics requests, but not *aggregate completed-request rate* — `handle_request` releases its permit as soon as one response closes, so a fast, cooperative peer can continuously reopen connections and complete full `GET /metrics` requests, driving unbounded task/formatting/TCP/bandwidth work without ever occupying all 64 permits. A distinct availability path from MAN-61 (which covers a *quiet*, permit-holding client — the opposite failure mode). Found in review round 7 | **Fixed** — per-source-IP accepted-connection budget on the metrics listener, charged once per connection regardless of whether it carries a complete request (`metrics_http.rs`, `MAX_METRICS_REQUESTS_PER_IP`/`METRICS_REQUEST_RATE_WINDOW`, configurable via `[server].metrics_max_requests_per_ip`), see `docs/DECISIONS/2026-09-04-man64-metrics-request-rate-and-source-health.md` |
...
| # | Category | Finding | Disposition |
|---|---|---|---|
| 16 | DoS | Both of `uplink.rs`'s inbound reads (`connect_and_forward`'s login-prompt read, `forward_loop`'s post-login discard read) use plain unbounded `AsyncBufReadExt::read_line`, unlike every other network-facing read in this codebase — an unterminated long line from the configured target can grow a buffer without bound. **Correction (review round 4): the other surfaces aren't all "routed through `bounded_io`"** as an earlier draft claimed — telnet and metrics HTTP are, but the raw JSON TCP path reads into a fixed 64-byte scratch buffer directly (`json_stream.rs:266`, no `bounded_io` involved), and WebSocket input is bounded through tungstenite's own frame/message-size configuration (`json_stream.rs:331-337`), a third distinct mechanism. The underlying point stands — every other surface enforces an explicit bound, `uplink.rs` doesn't — but by three different mechanisms, not one shared module | **Filed — MAN-58** |
...
| MAN-63 | The 16 KiB WS frame/message config only bounds post-upgrade traffic — the handshake itself has no size bound, only a 10s time bound, letting an unterminated oversized upgrade request buffer significant memory per connection across up to 512 concurrent connections | P2 — a real, previously-uncovered gap in an already-hardened surface; found in review round 6 |
| MAN-64 | Two round-7 findings, deferred per the convergence policy's round 6-15 tier (genuinely new P2s ticketed rather than fixed inline, to stop this PR's review loop from extending indefinitely): (1) `manta_source_health` is one-sided, never transitions to `false`; (2) the metrics endpoint has no bound on aggregate completed-request rate, only on simultaneous connections | P3 — both real but lower-severity: (1) is an observability accuracy gap (already noted inline in ARCHITECTURE.md, same treatment as `manta_active_tracks`), (2) requires a fast *cooperative* client, not an adversarial one exploiting a design flaw. **Resolved 2026-09-04**: (2) fixed outright (per-IP completed-request budget, see row 11a); (1) split — the one failure transition the daemon can actually observe (`manta_engine::listen` returning `Err`) is now wired before the shutdown-drain signal, while live staleness detection for a source degrading mid-run remains documented-and-deferred to MAN-56/MAN-13 (needs a per-read progress hook `listen` doesn't expose today) rather than fixed here — see `docs/DECISIONS/2026-09-04-man64-metrics-request-rate-and-source-health.md` and the updated `ARCHITECTURE.md` §8 note |
...
- No application-code changes were made from this ticket directly — every
  ...
  MAN-58, MAN-59, MAN-60, MAN-61, MAN-62, MAN-63, MAN-64) per the ticket's own invariant
...
- KiwiSDR/Soapy/Audio input drivers are not covered by this pass (review
  ...
  filed as MAN-60.
- Finding 1 is genuinely Fixed as of review round 4 — PR #75 merged and
  this branch now descends from that merge commit directly (superseded;
  an earlier round's "provisional on #75 merging" caveat no longer
  applies and would now be stale if left in place).
