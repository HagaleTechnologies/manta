...
|---|---|---|---|
| 1 | Tampering / DoS | A single malformed UDP packet (correct length, bad USB sync bytes) propagated a fatal `Err` all the way out of `manta-engine::listen`'s read loop via unmatched `?` — one crafted packet killed the whole process. **Also covers a related, initially-separate finding (review round 4): a sustained flood of short/wrong-length datagrams hit `Ok(_) => continue` with no bound at all** — `consecutive_timeouts` only advances on `WouldBlock`/`TimedOut`, never on a merely-short datagram, so `pump_one_packet` could loop indefinitely holding `Inner`'s shared mutex (stalling every DDC on the device) without ever timing out | **Fixed — PR #75, merged** (review round 4 correction: PR #75 has since merged, confirmed via `git log` on this branch after syncing with `main` — the earlier "not yet merged" disposition is now stale). Both the bad-sync-bytes case and the short/wrong-length-datagram flood case are bounded by the same fix: `MAX_CONSECUTIVE_MALFORMED` counts *both* discard paths and bails with a clear error after 10,000 consecutive malformed packets (round 1 of PR #75's own review added this bound and moved the keepalive check inside the loop, specifically closing the indefinite-mutex-hold gap) — the round-4 finding describes a real pre-fix bug, but one already covered by the same landed fix, not a separate undispositioned one. Regression-tested against a live loopback socket for both cases, both verified to fail against the pre-fix code |
