...
| RST(599 label)/QRL? message-content extraction | **Covered (JSON stream only)** — closed by MAN-33: `manta_spot::message` extracts the most recently decoded RST (including the `5NN` cut-number form) and a sticky-per-track QRL? flag, both carried on `Spot` and emitted on the JSON Lines/WebSocket stream (`rst`/`qrlQuery`, `crates/manta-server/src/spot_message.rs`). Deliberately **not** on the RBN telnet line (`rbn::format_line`) — matching CW Skimmer's own convention of showing these only in its band map, never in what it uploads; see `docs/DECISIONS/2026-09-09-man33-rst-qrl-spot-content.md` for the full design record and accepted parser limitations |
...
| MAN-33 | RST/QRL? spot-content extraction | **Closed.** `manta-spot/src/context.rs` classified CQ/DE/Beacon but had no RST or QRL? extraction, contra the original matrix's over-broad "covered" claim. Found by Codex review on PR #57, verified against source before filing. Implemented via `manta_spot::message` (JSON stream only); see `docs/DECISIONS/2026-09-09-man33-rst-qrl-spot-content.md` |
