...
| MAN-34 | RF center-frequency reference for the rig-audio input mode | **Covered as of 2026-09-05** — `AudioIqSource::with_center_freq_hz` + `listen`/`soak --dial-freq-hz`, see `docs/DECISIONS/2026-09-05-man-34-audio-rf-reference.md`. Originally filed because `AudioIqSource::center_freq_hz()` always returned `0.0` — the already-shipped `listen`/`listen --device` mode reported baseband offsets, not absolute frequency. Found by Codex review on PR #57 — the original CAT non-goal was correct for wideband sources but too broad to also cover this already-shipped narrowband mode |
