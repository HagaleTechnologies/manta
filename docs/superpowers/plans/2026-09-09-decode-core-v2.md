...
- **Out-of-scope guard**: no task edits `track.rs` or `gate.rs`; Task 13's call site is explicitly deferred. **Correction (final whole-branch review, post-execution):** this line's claim about `validator.rs` was wrong. Task 2's additive `WordBoundary`/`CharDecoded` struct-variant change (SPEC v2 §5) forces two compile-required touches to `validator.rs`: a `..` added to a struct-variant match arm, and test fixtures migrated to the new `char_decoded`/`word_boundary` constructors (commit `ba13eca`). This is unavoidable under Rust's struct-variant exhaustiveness rules given the additive-fields design — it is not a logic change to the validator — but it does mean the stated constraint was violated in fact, not just in this line's wording. See the SDD ledger's final-review entry for the full analysis; flagged in the PR description for the parallel session that owns this file.
