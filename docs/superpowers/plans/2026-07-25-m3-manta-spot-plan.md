# M3 sub-project 1 — `manta-spot` Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build `manta-spot`, a standalone, deterministic crate that turns a
`manta-decode` `DecoderEvent` stream into validated `Spot`s — callsign
...
- Design doc: `docs/superpowers/specs/2026-07-25-m3-manta-spot-design.md`
...
- Workspace convention: crate `Cargo.toml` uses `version.workspace = true`
  etc. (see any existing crate, e.g. `crates/manta-decode/Cargo.toml`);
...
- This sub-project stops at a fully tested standalone crate — no wiring
  into `manta-engine`, no `manta-server`, no TOML config loader. See
...
- Create: `crates/manta-spot/Cargo.toml`
- Create: `crates/manta-spot/src/lib.rs`
- Create: `crates/manta-spot/data/cty.dat`
- Create: `crates/manta-spot/data/master.scp`
- Create: `crates/manta-spot/data/SOURCES.md`

**Interfaces:**
- Produces: an empty-but-building `manta-spot` crate the rest of this
  plan fills in; `crates/manta-spot/data/cty.dat` and
  `crates/manta-spot/data/master.scp` as `include_str!`-able text assets
  Task 4/5 consume.

- [ ] **Step 1: Register the crate and its new dependency in the workspace**

Edit root `Cargo.toml`:
- Add `"crates/manta-spot",` to `members`.
- Add `manta-spot = { path = "crates/manta-spot" }` to
  `[workspace.dependencies]` (alongside the other `manta-*` entries).
- Add `regex = "1"` to `[workspace.dependencies]` (alongside `serde`,
  `anyhow`, etc.).

- [ ] **Step 2: Create the crate manifest**

`crates/manta-spot/Cargo.toml`:
...
curl -sf --max-time 30 "https://www.country-files.com/cty/cty.dat" \
  -o crates/manta-spot/data/cty.dat
curl -sf --max-time 30 "https://www.supercheckpartial.com/MASTER.SCP" \
  -o crates/manta-spot/data/master.scp
wc -l crates/manta-spot/data/cty.dat crates/manta-spot/data/master.scp
```
...
`crates/manta-spot/data/SOURCES.md`:
...
```bash
git add Cargo.toml crates/manta-spot
git commit -m "feat(spot): scaffold manta-spot crate, vendor cty.dat/master.scp"
```
...
**Files:**
- Create: `crates/manta-spot/src/grammar.rs`
...
`crates/manta-spot/src/grammar.rs`:
...
Add to `crates/manta-spot/src/lib.rs`:
...
```bash
git add crates/manta-spot/src/lib.rs crates/manta-spot/src/grammar.rs
...
**Files:**
- Create: `crates/manta-spot/src/context.rs`
- Modify: `crates/manta-spot/src/lib.rs`
...
`crates/manta-spot/src/context.rs`:
...
```bash
git add crates/manta-spot/src/lib.rs crates/manta-spot/src/context.rs
...
**Files:**
- Create: `crates/manta-spot/src/cty.rs`
- Modify: `crates/manta-spot/src/lib.rs`
...
```bash
git add crates/manta-spot/src/lib.rs crates/manta-spot/src/cty.rs
...
**Files:**
- Create: `crates/manta-spot/src/scp.rs`
- Modify: `crates/manta-spot/src/lib.rs`
...
```bash
git add crates/manta-spot/src/lib.rs crates/manta-spot/src/scp.rs
...
**Files:**
- Create: `crates/manta-spot/src/confidence.rs`
- Modify: `crates/manta-spot/src/lib.rs`
...
`crates/manta-spot/src/confidence.rs`:
...
Add to `crates/manta-spot/Cargo.toml`:
...
```bash
git add crates/manta-spot/Cargo.toml crates/manta-spot/src/lib.rs crates/manta-spot/src/confidence.rs
...
**Files:**
- Create: `crates/manta-spot/src/gate.rs`
- Modify: `crates/manta-spot/src/lib.rs`
...
```bash
git add crates/manta-spot/src/lib.rs crates/manta-spot/src/gate.rs
...
**Files:**
- Create: `crates/manta-spot/src/dedupe.rs`
- Modify: `crates/manta-spot/src/lib.rs`
...
`crates/manta-spot/src/dedupe.rs`:
...
```bash
git add crates/manta-spot/src/lib.rs crates/manta-spot/src/dedupe.rs
...
**Files:**
- Create: `crates/manta-spot/src/validator.rs`
- Modify: `crates/manta-spot/src/lib.rs`

**Interfaces:**
- Consumes: `manta_decode::events::DecoderEvent`,
  `manta_decode::tree::{Glyph, Prosign}` (existing crate);
...
- Produces: `pub struct Spot { callsign: String, freq_hz: f64, snr_db: f32,
  ...
  future `manta-engine` wiring use it.

- [ ] **Step 1: Write the module**

`crates/manta-spot/src/validator.rs`:
...
Replace the contents of `crates/manta-spot/src/lib.rs` with:
...
- Create: `crates/manta-spot/tests/golden_v11_v15.rs`

**Interfaces:**
- Consumes: `manta_spot::{Validator, Spot, SpotType}` (Task 9),
  `manta_decode::events::DecoderEvent`, `manta_decode::tree::Glyph`
  (existing crate).
...
Run: `cargo test -p manta-spot --test golden_v11_v15`
...
```bash
git add docs/SPEC-decode-core.md crates/manta-spot/tests/golden_v11_v15.rs
...
In `ARCHITECTURE.md` §2's dependency graph, `manta-spot` needs
`DecoderEvent`/`Glyph` from `manta-decode` — an edge the original graph
...
git commit -m "docs(m3): manta-spot dependency edge + ROADMAP status"
```
...
```bash
git push
gh pr create --title "feat(spot): M3 sub-project 1 -- manta-spot validation crate" --body "$(cat <<'EOF'
## Summary
- New `manta-spot` crate: callsign grammar, CQ/DE/beacon context parse,
...
- Vendors real `cty.dat` (AD1C) and `master.scp` (Super Check Partial)
  snapshots; provenance in `crates/manta-spot/data/SOURCES.md`.
- Adds SPEC-decode-core.md §7.1 V11-V15 validator golden vectors.
- Out of scope (follow-up sub-projects): wiring into `manta-engine`,
  `manta-server`, TOML config, RBN parity benchmark.

Design: `docs/superpowers/specs/2026-07-25-m3-manta-spot-design.md`
Plan: `docs/superpowers/plans/2026-07-25-m3-manta-spot-plan.md`

## Test plan
- [x] `cargo test -p manta-spot` (unit tests, every module)
- [x] `cargo test -p manta-spot --test golden_v11_v15` (V11-V15)
