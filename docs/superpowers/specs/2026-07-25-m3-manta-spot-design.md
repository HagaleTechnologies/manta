# M3 sub-project 1 — `manta-spot` validation crate: Design
...
way M2 did: this sub-project builds `manta-spot` (callsign/CQ-DE
validation, dedupe/scoring) as a standalone, fully tested crate.
`manta-server` (telnet + JSON/WS output), TOML config loading, the wiring
of `manta-spot` into `manta-engine`'s live pipeline, and the RBN parity
...
- New workspace crate `manta-spot`, implementing ARCHITECTURE §6's
...
- Wiring `manta-spot` into `manta-engine`'s live pipeline, and swapping
...
- `manta-server` (telnet cluster + JSON Lines/WebSocket), TOML config
  loading, metrics endpoint — none of `manta-spot`'s output has a
  transport yet; that's `manta-server`'s job.
...
Bundled as `crates/manta-spot/data/cty.dat` (`include_str!` at build
...
license recorded in `crates/manta-spot/data/SOURCES.md`. Binary search
...
Bundled `crates/manta-spot/data/master.scp` snapshot (same
...
`SpeedUpdate`, `TrackMeta`) — `manta-spot` takes a dependency on whatever
crate currently defines it (`manta-engine`, per current layout) rather
...
sample_ts / fs`) happens at the `manta-server` boundary per SPEC §5, not
here.
...
Once this lands, ROADMAP.md's M3 section notes `manta-spot`'s validation
crate as complete, with engine wiring, `manta-server`, and the parity
