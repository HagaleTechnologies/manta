...
`skimmer-dsp` and `skimmer-decode` from this file without making design
...
in `skimmer-input` to the nearest table rate before the channelizer;
...
hops at `fo = 375 Hz` exactly once, at startup. `fo` is invariant across input
...
prototype designer is therefore new code in `skimmer-dsp::proto` (~40 lines:
...
- Quadratic interpolation on **dB** powers of `(k₀−1, k₀, k₀+1)`:
  ...
  Clamp `δ_m` to `[−0.5, +0.5]`; if the denominator ≤ 0 (no local max), set
  `δ_m = 0` and mark the hop unusable.
...
- Key-down when `a[m] > 1.25·T`; key-up when `a[m] < 0.80·T`
  ...
  holds.
- **Debounce:** a run (mark or space) shorter than **12 ms (4 hops)** is
...
(cty.dat / SCP context in `skimmer-spot`), and a character-local beam makes
...
4. At the character boundary: among surviving hypotheses, those whose node
   has no glyph score an additional **−3.0** penalty and map to their nearest
   glyph ancestor... no — they are simply dropped; if all four are glyphless,
   the character emits `?` with confidence 0. The winner is the highest-score
   glyph-bearing hypothesis.
...
### 4.6 Per-callsign confidence (consumed by `skimmer-spot`)
...
and are specified in `skimmer-spot`, not here. The ≥ 2-repetition gate for
first spot is unchanged.
...
(`skimmer-server`), derived as `stream_start_time + sample_ts / fs` where
...
1. **No RNG** anywhere in `skimmer-dsp` / `skimmer-decode`. (The testkit's
...
All generated by `skimmer-testkit`: text → keyed envelope (raised-cosine
...
(`awgn(seed)`, `watterson(preset, seed)`). SNR is quoted **in 2500 Hz**.
...
| V6 | qsb-sine | 20 WPM, envelope ×(0.55 + 0.45·sin 2π·0.2t) (≈ +20→+2 dB), K5ZZZ | AWGN | char ≥ 90 %; track survives (no CLOSED before end) |
...
M0 = V1 passing end-to-end from a WAV file. M1 = V1–V6. V7–V10 gate M2
...
|---|---|
| §1 channelizer, prototype | `skimmer-dsp::pfb`, `skimmer-dsp::proto` |
| §1.4 interpolation | `skimmer-dsp::centroid` |
| §2 floor + gate + state machine | `skimmer-dsp::floor`, `skimmer-engine::track` |
| §3 demod | `skimmer-decode::envelope` |
| §4.1–4.2 timing | `skimmer-decode::timing` |
| §4.3–4.5 beam decode | `skimmer-decode::beam`, `skimmer-decode::tree` |
| §5 events | `skimmer-decode::events` |
| §7 vectors | `skimmer-testkit::vectors` |
