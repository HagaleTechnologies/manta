...
=======
- `Validator::tracks` is per-track_id state that must be freed on `DecoderEvent::TrackClosed` — the normative teardown contract (a real, measured leak this bug produced) lives in `docs/DECISIONS/2026-09-02-man19-track-closed-teardown-invariant.md`, not here. `RepetitionGate::seen` is *not* per-track_id state (MAN-166): it's keyed by frequency bucket and forgotten by periodically sweeping elapsed time (`RepetitionGate::sweep`), not by any single track closing — freeing it on `TrackClosed` used to defeat its own 90s repetition window the instant a real signal's track churned.
