...
- `decode_samples`'s single-track `.text`/`.wpm`/`.freq_hz` report the track with the most *rendered* characters (glyphs that survive into `events_to_text`; prosigns and detector-only `TrackPromoted` events do not count), not the lowest `track_id` — same DECISIONS doc as above, which is normative.
...
- Eviction bounds each lifecycle class separately: promoted (ACTIVE/HANG) tracks against `track_cap`, unconfirmed CANDIDATEs against `candidate_cap`, each thinned by dropping its own lowest-`current_snr_db` member. A candidate is therefore never evicted in a decoding track's place, and — the round-7 half of the rule — a saturated cap is *not* a reason to kill newborn candidates, so a stronger signal can still confirm and then take the weakest incumbent's slot on SNR (normative deviation from ARCHITECTURE §4's literal single lowest-SNR cap: SPEC §2.4, derived in the MAN-3 DECISIONS doc above). Merges (SPEC §2.5) and evictions (ARCHITECTURE §4) are counted in-process via `TrackManager::close_counts` (issue #26) — **no silent coverage loss**, though external exposition as Prometheus metrics is explicit M3 scope (ARCHITECTURE §8), not yet wired.
