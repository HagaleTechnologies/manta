...
fn main() -> Result<()> {
    ...
    match Cli::parse().command {
        ...
        Command::Run {
            ...
            // MAN-122 supersedes MAN-45's `ListenObservers::active_tracks`
            // atomic + 4 Hz poller here: the `on_tracks` callback below
            // publishes `TrackManager`'s PROMOTED-track count synchronously
            // with the decode loop, so `manta_active_tracks` (and the status
            // line's `tracks=`) no longer counts unconfirmed CANDIDATEs, no
            // longer lags a poll interval, and cannot be republished stale by
            // a tick racing shutdown. MAN-45's observer API stays in
            // `manta-engine` for a consumer that must not be able to block the
            // decode loop; this daemon's callback is cheap enough (two relaxed
            // atomics per ~43 ms chunk) that it isn't one.
            ...
            if let Some(server) = &spot_server {
                ...
                // MAN-122 merge: the gauge must read 0 for the whole
                // shutdown drain, so a metrics scrape landing anywhere in the
                // `SHUTDOWN_DRAIN_DEADLINE` window that follows never sees a
                // stale nonzero count for a daemon with no live tracks.
                // `listen_with_track_count`'s observer already publishes 0 on
                // both exits (`tm.finish()`'s trailing zero on success, and
                // the explicit zero before a failed `IqSource::read`
                // propagates) -- but `listen_result` above is deliberately
                // captured rather than `?`-ed so this drain still runs on an
                // error path (round-7 finding), and this write is what makes
                // the zero unconditional rather than dependent on the engine
                // having reached either of those points. No poller to abort
                // first anymore: the observer is synchronous with the decode
                // loop, which has already returned here, so nothing can race
                // this store.
