...
pub fn render_scene(
    ...
    for sig in signals {
        ...
        if let Some(fade) = sig.watterson {
            ...
            // cannot move the V1-V10 golden baseline. Every golden offset
            // is >= 5.6 kHz, where 129 taps already gives >= 94 dB image
            // rejection -- this path never needed the widening.
