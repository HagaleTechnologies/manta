...
- `vp8geo_cw_48khz.wav` (30 MB, committed) — the loadable fixture. Mono
  PCM16 @ 48 kHz, converted from `vp8geo_cw.mp3` via:
...
- `vp8geo_cw.mp3` (1.2 MB, committed) — the original capture, kept for
  provenance/reference. ~5 min CW pileup on VP8GEO, listener tuning around
  within an SSB-bandwidth (~2.4-3 kHz) capture, with wideband noise
  present. Source: George, 2026-09-02.
- `wpx_cw_iq_96khz.wav` (389 MB, **not committed**, see `.gitignore`) — WPX
  ...
  before re-adding via LFS (storage cost). Also not format-compatible as-is
  (96 kHz vs. the required 48 kHz) if it's ever brought in.
...
material for the manta audio corpus. **Redistribution rights are not yet
...
`crates/manta-spot/data/SOURCES.md`, this entry does not yet give
downstream users a basis for redistributing the recording. Needs an
explicit permission/license statement from the source before this can be
considered settled — see the open PR discussion.
