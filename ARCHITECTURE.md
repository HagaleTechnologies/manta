...
- **`tracing` + `tracing-subscriber` with `EnvFilter`, implemented for
  ...
  Prometheus text endpoint (the "(feature `metrics`)" phrasing in older revisions of this
  doc was stale — no Cargo `metrics` feature has ever existed; the
  endpoint is unconditionally compiled and served whenever `--config` is
  set — `--server-config` is MAN-77's deprecated alias of that flag):
  active tracks, evictions, decode rate, spots/min, per-stage queue
  depths, spot confidence histogram — still aspirational for several of
  these fields; the currently-implemented subset is `manta_spots_total`,
